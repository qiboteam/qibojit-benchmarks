...
def plot_scaling_main(cpu_data, gpu_data, circuit, quantity, precision="complex128", fontsize=30, legend=True, save=False):
    matplotlib.rcParams["font.size"] = fontsize
    # Prepare GPU data
    condition = (gpu_data["circuit"] == circuit) & (gpu_data["precision"] == precision)
    backends = ["tensorflow", "qibotf"]
    data = {f"{k} GPU": gpu_data[(gpu_data["backend"] == f"{k}") & condition] for k in backends}
    data["qibojit cupy GPU"] = gpu_data[(gpu_data["backend"] == "qibojit") & (gpu_data["platform"] == "cupy") & condition]
    data["qibojit cuquantum GPU"] = gpu_data[(gpu_data["backend"] == "qibojit") & (gpu_data["platform"] == "cuquantum") & condition]

    # Prepare CPU data
    backends = ["numpy", "tensorflow", "qibotf", "qibojit"]
    condition = (cpu_data["circuit"] == circuit) & (cpu_data["precision"] == precision)
    data.update({k: cpu_data[(cpu_data["backend"] == f"{k}") & condition] for k in backends})
